...
app.controllerProvider.register('vote_view', function($scope, $http, $location) {
    ...
    $http.get('supersede-dm-assetmgr/playermove/' + $scope.playerMoveId)
    .success(function(data) {
     ...
     $scope.insertPlayerVote = function(){
         $http.put('supersede-dm-assetmgr/playermove/' + $scope.playerMoveId + '/vote/' + $scope.selectedRequirementsChoice.selected)
            .success(function(data) {
                $location.url('/supersede-dm-assetmgr/player_moves?gameId=' + data);
        });
     };
});