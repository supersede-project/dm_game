...
app.controllerProvider.register('games', function($scope, $http, $location) {
	...
	$http.get('supersede-dm-assetmgr/game').
	success(function(data) 
	{
		...
		$scope.openSettings =
		{
			...
			columns: [
			    ...
			    { text: '', width: '10%', datafield: 'gameId', cellsRenderer: function (row, columnDataField, value) {
					var r = '<div class="jqx-grid-cell-left-align" style="margin-top: 4px; margin-bottom: 4px;">';
					r = r.concat("<jqx-link-button jqx-width='75' jqx-height='25'><a href='#/supersede-dm-assetmgr/game_page?gameId=" + value + "'>Detail</a></jqx-link-button>");
					return r.concat("</div>");
					}
			    }
			]
	    ...
	    $('#jqxTabs').on('tabclick', function (event) {
	        if (event.args.item == $('#unorderedList').find('li').length - 1) {
	        	...
	        	$location.path('/supersede-dm-assetmgr/create_game');
	        }
	    });
    ...
    $scope.goGame = function (gameId) {
    	$location.path('/supersede-dm-assetmgr/game_page').search('gameId', gameId);
	};
