...
app.controllerProvider.register('player_moves', function($scope, $http, $location, $interval,  $rootScope) {
	...
	getActions = function() {
		$http.get('supersede-dm-assetmgr/playermove', {params: { criteriaId: $scope.selectedCriteria, gameId: $scope.selectedGame, gameNotFinished: true }}).success(function(data) {
			...
			$scope.openSettings =
			{
				...
				columns: [
					...
					{ text: '', width: '7%', datafield: 'playerMoveId2', cellsRenderer: function (row, columnDataField, value) {
						var r = '<div class="jqx-grid-cell-left-align" style="margin-top: 4px; margin-bottom: 4px;">';
						r = r.concat("<jqx-link-button jqx-width='55' jqx-height='25'><a href='#/supersede-dm-assetmgr/vote_view?playerMoveId=" + value + "'>Vote</a></jqx-link-button>");
						return r.concat("</div>");
						}
				    }
				]
	...
	$scope.setVote = function(playerVote, playerMoveId){
		$http.put('supersede-dm-assetmgr/playermove/' + playerMoveId + '/vote/' + playerVote).success(function(data) {
			getActions();
		});
	};
	...
	$scope.openMove = function(playerMoveId){
		$http.put('supersede-dm-assetmgr/playermove/open/' + playerMoveId).success(function(data) {
			getActions();
		});
	};
	...
	{
		$http.get('supersede-dm-assetmgr/game/' + $scope.selectedGame).success(function(data) {
			$scope.game = data;	
		...
		$http.get('supersede-dm-assetmgr/user/current')
		.success(function(data) {
			$scope.user = data;
		});
		
		$http.get('supersede-dm-assetmgr/gameplayerpoint/game/' + $scope.selectedGame)
		.success(function(data) {
