...
app.controllerProvider.register('create_game', function($scope, $http, $location) {
    ...
    $http.get('supersede-dm-assetmgr/user?profile=OPINION_PROVIDER')
	...
	{
		$http({
			url: "supersede-dm-assetmgr/game",
	        data: $scope.game,
	        method: 'POST',
	    	...
	    	$scope.choices = {};
	    	$scope.currentPage = 'page1';
	    	$location.url('supersede-dm-assetmgr/game_page').search('gameId', data);
	    }).error(function(err){
	    	console.log(err);
	    });
	};
