...
app.controllerProvider.register('judge_acts', function($scope, $http) {
	...
	getActs = function() {
		$http.get('supersede-dm-assetmgr/judgeact', {params: { criteriaId: $scope.selectedCriteria, gameId: $scope.selectedGame, gameNotFinished: true }}).success(function(data)
		{
			...
			$scope.openSettings =
			{
				...
				columns: [
					...
					{ text: '', width: '7%', datafield: 'judgeActId2', cellsRenderer: function (row, columnDataField, value) {
						var r = '<div class="jqx-grid-cell-left-align" style="margin-top: 4px; margin-bottom: 4px;">';
						r = r.concat("<jqx-link-button jqx-width='55' jqx-height='25'><a href='#/supersede-dm-assetmgr/judge_act?judgeActId=" + value + "'>Decide</a></jqx-link-button>");
						return r.concat("</div>");
						}
				    }
				]
	...
	$scope.setVote = function(judgeVote, judgeActId){
		$http.put('supersede-dm-assetmgr/judgeact/' + judgeActId + '/vote/' + judgeVote).success(function(data)
		{
			getActs();
		});
	};
